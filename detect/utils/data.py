...
class Data(object):
    def __init__(self):
        ...
        self.__annotations = self.__load_annotations2('/home/user/datasets/dataset/CCPD/home/booy/booy/ccpd_dataset/ccpd_base')
    ...
    def __load_annotations(self, data_path, use_difficult_bbox=False):
        ...
        return annotations
    def __load_annotations2(self, data_path, use_difficult_bbox=False):
        """
        :param data_path: 数据集的路径，如'/home/xzh/doc/code/python_code/data/VOC/2012_trainval'
        :return: 一个标注列表，元素为：'image_path xmin,ymin,xmax,ymax,class_ind xmin,ymin,xmax,ymax,class_ind ...'
        """
        file_names=os.listdir(data_path)
        file_pathes=[data_path+'/'+f for f in file_names]
        annotations = []
        for fn,fp in zip(file_names,file_pathes):
            bbox=fn.split('.')[0].split('-')[2]
            [ab,cd]=bbox.split('_')
            [xmin,ymin]=ab.split('&')
            [xmax,ymax]=cd.split('&')
            class_ind=0
    ...
    def __parse_annotation(self, annotation):
        ...
        image, bboxes = dataAug.random_vertical_flip(np.copy(image), np.copy(bboxes))
        image, bboxes = dataAug.random_rotation90_flip(np.copy(image), np.copy(bboxes))
