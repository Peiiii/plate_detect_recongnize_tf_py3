...
class Evaluator(object):
    ...
    def __APs_calc(self, iou_thresh=0.5, use_07_metric=False):
        ...
        annopath = os.path.join(self._dataset_path, 'Annotations', '{:s}.xml')
        imagesetfile = os.path.join(self._dataset_path, 'ImageSets', 'Main', 'test.txt')
        APs = {}
        for i, cls in enumerate(cfg.CLASSES):
            rec, prec, ap = voc_eval.voc_eval(filename, annopath, imagesetfile, cls, cachedir, iou_thresh,
                                              use_07_metric)
            APs[cls] = ap
    ...
    def APs_voc(self, year=2007, multi_test=False, flip_test=False):
        ...
        test_set_path = self._dataset_path
    ...
    def testall(self, year=2007, multi_test=False, flip_test=False):
        """
        :param year: 使用哪个数据集测试
        :param multi_test: 是否使用多尺度测试
        :param flip_test: 是否使用水平翻转测试
